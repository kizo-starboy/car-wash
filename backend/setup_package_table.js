// Simple script to add Package table to existing CWSMS database
...
console.log('🚀 Setting up Package table...');

connection.connect((err) => {
  if (err) {
    console.error('❌ Database connection failed:', err.message);
    console.log('Please make sure:');
    console.log('1. MySQL/MariaDB is running');
    console.log('2. Database "cwsms" exists');
...
function verifySetup() {
  connection.query('SELECT COUNT(*) as count FROM package', (err, results) => {
    ...
    connection.query('DESCRIBE servicepackage', (err, results) => {
      ...
      console.log('\n📋 What was done:');
      console.log('   ✓ Package table created with auto-increment PackageNumber');
      console.log('   ✓ 3 default packages inserted (Basic, Premium, Deluxe)');
      ...
      console.log('   - Add new packages through the web interface');
      console.log('   - Edit existing packages');
      console.log('   - Create service records with package selection');
